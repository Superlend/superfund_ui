...
import { TOTAL_ALLOCATION_POINTS, USDC_DECIMALS, VAULT_ADDRESS, VAULT_STRATEGIES } from '@/lib/constants';
...
export function useVaultAllocationPoints() {
    ...
    const [allocationPoints, setAllocationPoints] = useState<{ name: string, value: number }[]>([]);
    ...
    async function fetchAllocationPoints() {
        ...
        for (const key of keys) {
            const strategies_internal = strategies[key as keyof typeof strategies];
            ...
            for (const strategy of strategies_internal) {
                calls.push(publicClient.readContract({
                    ...
                    local_allocationPoints.push({ name: key, value: Number(res[1].toString()) })
        ...
        local_allocationPoints = local_allocationPoints.map(item => ({
            name: item.name,
            value: item.value != 0 ? parseFloat(((Number(item.value) / TOTAL_ALLOCATION_POINTS) * 100).toFixed(2)) : 0
        }))

        // if the key same then add the value
        local_allocationPoints = local_allocationPoints.reduce((acc: { name: string, value: number }[], item) => {
            const existing = acc.find(i => i.name === item.name);
            if (existing) {
                existing.value = parseFloat((Number(existing.value) + Number(item.value)).toFixed(2));
            } else {
                acc.push(item);
            }
            return acc;
