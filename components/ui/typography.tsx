...
import {
    ...
    IBodyTextProps,
    ILabelProps,
} from '../../interfaces/ui/ITypography'

const COMMON_STYLES = ''

const hasValidHeadingLevel = (
    level: string
): level is 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' => {
...
const getHeadingClassName = (
    level: IHeadingTextProps['level'] = 'h1',
    weight: IHeadingTextProps['weight'] = 'bold',
    className: string = ''
) => {
...
const getBodyClassName = (
    level: IBodyTextProps['level'] = 'body1',
    weight: IBodyTextProps['weight'] = 'normal',
    className: string = ''
) => {
...
export function BodyText({
    ...
    return (
        <p className={getBodyClassName(level, weight, className)} title={title}>{children}</p>
    )
}
...
const getLabelClassName = (
    size: ILabelProps['size'] = 'small',
    weight: ILabelProps['weight'] = 'normal',
    className: string = ''
) => {
...
export function Label({
    ...
    return (
        <label
            className={getLabelClassName(size, weight, className)}
            htmlFor={htmlFor}
            {...props}
        >
            {children}
