      ...
      {hasTransactions && (
        <div className="flex flex-row justify-between items-start md:items-center mb-6 gap-4 md:gap-0">
          ...
          <div className="flex items-center gap-3 w-auto">
            ...
            <div className="hidden md:flex items-center gap-1 p-1 bg-muted/50 rounded-lg border w-full md:w-auto">
              <button
                onClick={() => setCurrentFilter('all')}
                className={`relative px-4 py-2 text-xs font-medium rounded-md transition-all duration-300 flex-1 md:flex-none overflow-hidden min-w-[60px] ${currentFilter === 'all'
                    ? 'text-white shadow-sm'
                    : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                <div className={`absolute inset-0 bg-blue-500 rounded-md transition-all duration-300 ease-out ${
                  currentFilter === 'all' 
                    ? 'scale-100 opacity-100' 
                    : 'scale-0 opacity-0'
                }`} />
                <div className="relative z-10 flex items-center justify-center gap-2">
                  <div className={`w-2 h-2 rounded-full bg-blue-500 transition-all duration-500 ease-out ${
                    currentFilter === 'all' 
              ...
              <button
                ...
                className={`relative px-4 py-2 text-xs font-medium rounded-md transition-all duration-300 flex-1 md:flex-none overflow-hidden min-w-[80px] ${currentFilter === 'deposit'
                    ? 'text-white shadow-sm'
                    : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                <div className={`absolute inset-0 bg-green-500 rounded-md transition-all duration-300 ease-out ${
                  currentFilter === 'deposit' 
                    ? 'scale-100 opacity-100' 
                    : 'scale-0 opacity-0'
                }`} />
                <div className="relative z-10 flex items-center justify-center gap-2">
                  <div className={`w-2 h-2 rounded-full bg-green-500 transition-all duration-500 ease-out ${
                    currentFilter === 'deposit' 
                      ? 'scale-0 opacity-0 hidden' 
                      : 'scale-100 opacity-100'
                  }`} />
              ...
              <button
                ...
                className={`relative px-4 py-2 text-xs font-medium rounded-md transition-all duration-300 flex-1 md:flex-none overflow-hidden min-w-[100px] ${currentFilter === 'withdraw'
                    ? 'text-white shadow-sm'
                    : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                <div className={`absolute inset-0 bg-red-500 rounded-md transition-all duration-300 ease-out ${
                  currentFilter === 'withdraw' 
                    ? 'scale-100 opacity-100' 
                    : 'scale-0 opacity-0'
                }`} />
                <div className="relative z-10 flex items-center justify-center gap-2">
                  <div className={`w-2 h-2 rounded-full bg-red-500 transition-all duration-500 ease-out ${
                    currentFilter === 'withdraw' 
                      ? 'scale-0 opacity-0 hidden' 
                      : 'scale-100 opacity-100'
                  }`} />
                ...
                <DropdownMenuContent align="end" className="w-56 p-2">
                  <div className="space-y-2">
                    ...
                    <DropdownMenuGroup className="space-y-1">
                                          <DropdownMenuItem 
                      onClick={() => setCurrentFilter('all')}
                      className={`relative rounded-md cursor-pointer transition-all duration-200 overflow-hidden ${currentFilter === 'all' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'hover:bg-accent/50'
                        }`}
                    >
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full bg-blue-500 transition-all duration-200 ${
                            currentFilter === 'all' ? 'scale-125' : 'scale-100'
                          }`}></div>
                    ...
                    <DropdownMenuItem 
                      onClick={() => setCurrentFilter('deposit')}
                      className={`relative rounded-md cursor-pointer transition-all duration-200 overflow-hidden ${currentFilter === 'deposit' ? 'bg-green-50 text-green-700 border border-green-200' : 'hover:bg-accent/50'
                        }`}
                    >
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full bg-green-500 transition-all duration-200 ${
                            currentFilter === 'deposit' ? 'scale-125' : 'scale-100'
                          }`}></div>
                    ...
                    <DropdownMenuItem 
                      onClick={() => setCurrentFilter('withdraw')}
                      className={`relative rounded-md cursor-pointer transition-all duration-200 overflow-hidden ${currentFilter === 'withdraw' ? 'bg-red-50 text-red-700 border border-red-200' : 'hover:bg-accent/50'
                        }`}
                    >
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full bg-red-500 transition-all duration-200 ${
                            currentFilter === 'withdraw' ? 'scale-125' : 'scale-100'
                          }`}></div>
          ...
          Object.entries(groupedTransactions).map(([dateGroup, txs]) => (
            <div key={dateGroup} className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 text-sm font-semibold text-foreground bg-gradient-to-r from-accent/10 to-accent/5 py-2 px-4 rounded-lg border border-accent/20">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={copyTxHash}
                        className="group/copy text-xs text-orange-500 hover:text-orange-600 active:text-orange-700 flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-1 rounded-md hover:bg-orange-50 active:bg-orange-100 transition-all duration-200 min-h-[32px] sm:min-h-auto"
                      >
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Link
                        ...
                        target="_blank"
                        className="group/link text-xs text-orange-500 hover:text-orange-600 active:text-orange-700 flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-1 rounded-md hover:bg-orange-50 active:bg-orange-100 transition-all duration-200 min-h-[32px] sm:min-h-auto"
                      >
          ...
          <div className="flex flex-col items-end gap-1.5 ml-2 sm:ml-3">
            {type === 'deposit' ? (
              <>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg bg-red-50/70 border border-red-100/70 hover:bg-red-50 hover:border-red-200 transition-colors duration-200 cursor-pointer">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg bg-green-50/70 border border-green-100/70 hover:bg-green-50 hover:border-green-200 transition-colors duration-200 cursor-pointer">
